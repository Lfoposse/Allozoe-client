...
class Option{
  ...
  Option.map(dynamic obj) {
    ...
    this._complements =  (obj['products'] as List).map((item) => new Complement.map(item)).toList();
  }
  ...
  Option copy(){
    ...
    option.complements = _complements.asMap().map((index, complement) => new MapEntry(index, complement.copy())).values.toList();
  ...
  Map<String, dynamic> toMapRequest() {
    ...
    for (Complement complement in _complements) complements.add(complement.toMapRequest());
